...
import (
	...
	"mainModule/grpc/lukeFrame/metric"
...
func headersToContext(ctx context.Context, r *http.Request) context.Context {
	...
	for k := range r.Header {
		...
		ctx = context.WithValue(ctx, types.LukeString(k), r.Header.Get(k))
		ctx = context.WithValue(ctx, types.LukeString(strings.ToLower(k)), r.Header.Get(k))
	}

	// Tune specific change.
	// also add the request url
	ctx = context.WithValue(ctx, types.LukeString("request-url"), r.URL.Path)
	ctx = context.WithValue(ctx, types.LukeString("transport"), "HTTPJSON")

	return ctx
}
func remoteAddrToContext(ctx context.Context, r *http.Request) context.Context {
	println("---- transport_http remoteAddrToContext ----")
	return context.WithValue(ctx, types.LukeString("http-remote-addr"), r.RemoteAddr)
}
